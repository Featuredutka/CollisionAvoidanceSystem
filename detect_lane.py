...
#    cv.imshow('lines',detected_image)
    return detected_image, left, right

# function to call and perform all operations on each frame
def process(image):
    # to detect dark lines on the light floor
...
#    cv.imshow('edges',edges)
    ...
    final_image, left, right = draw_lines(left_lane, right_lane, image.copy())
    return final_image, left, right
